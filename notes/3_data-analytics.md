...
# Machine Learning
- Machine learning is an applicaiton of artificial intelligence (AI) that provides systems the ability to automatically learn and improve from experience without being explicitly programmed.
- Machine learning focuses on the development of 'computer programs that can access data and use it learn for themselves'

# Useful Terminology
- Features: The number of features or distinct traits that can be used to describe each item in a quantitive manner
...
1. Define and Initialise a Model
...
## Linear Regression
- Independent Variables (features): An independent variable is a variable that is manipulated to determine the value of a dependant variable. Simply, they are the features which we want to use to predict some given value of Y. It can be also called explanatory variable.
...
- f: funciton that describes the relationship between X and Y
...
- The KNN classifier is a non parameteric and instance-bsed learning algorithm
- Non-parameteric means it makes no explicit assumptions about the functional form of how the prediction is made, avoiding the dangers of mismodeling the underlying distribution of the data.
- Instance-bsed learning means that our algorithm doesn't explicitly learn a model. Instead, it chooses to memorize the training instances which are subsequently used as knowledge for the prediction phase.
## Algorithm
Give a training set X_train with labels Y_train, given a new instance X_test to be classified:
1. Find the most similiar instances  (let's call them X_NN) to X_test that are in X_train.
...
2. How many nearest neighbours to look at (eg. Five)
3. Optional weighting function on the neighbours points (eg. closer points are weighted higher than farther points)
4. How to aggregate the clsases of neighbours points (ie. simple majority voting)

![KNN](images/knn.png)

## Other notes
- Each row in the dataset refers to an 'instance'
- Remember to inspect your data

# Generalisation, Overfitting and Underfitting
- Generalisation ability refers to an algorithm's ability to give accurate predictions for new, previously unseen data.
- Assumptions:
  ...
  - But that may not happen if the trained model is truned too specifically to the training set.
...
- For comparison, if we have a "dummy" classifier that doesn't condser the features at all but rather blindly predict according to the most frequent class.
- If we use the same dataset mentioned in the previous slide (1000 data instance with 999 negative and 1 positive).
...
- Initialize
    ...
    - Pick k random points to serve as initial guess of the cluster centres
- Step A:
    - Assign each data point to the nearest cluster centre
- Step B:
    - Update each cluster centre by replacing it with the mean of all points assigned to that centre (from step A)
- Iterate:
    - Iterate over steps A and B until centres converge to a stable solution

## Limitation of k-means
- Sometimes the number of clisters is difficuilt to determine
