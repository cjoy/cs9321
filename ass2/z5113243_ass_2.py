...
@api.route(f'/{COLLECTION}/<collection_id>', endpoint=f'{COLLECTION}_byid')
@api.param('collection_id', f'Unique id, distinguishing {COLLECTION}.')
...
@api.route(f'/{COLLECTION}/<collection_id>/<date>/<country>', endpoint=f'{COLLECTION}_countrydate')
@api.param('collection_id', f'Unique id, distinguishing {COLLECTION}.')
@api.param('date', 'Year (ranging from 2012 - 2017.)')
...
class CollectionByCountryYear(Resource):
  ...
  @api.response(200, 'Successfully retrieved economic indicator for given country.')
...
@api.route(f'/{COLLECTION}/<collection_id>/<date>', endpoint=f'{COLLECTION}_topbottom')
@api.param('collection_id', f'Unique id, distinguishing {COLLECTION}.')
@api.param('date', 'Year (ranging from 2012 - 2017.)')
